...
pub mod erc721uri_storage {
    ...
    impl<M> ::core::fmt::Debug for ERC721URIStorage<M> {
        fn fmt(&self, f: &mut ::core::fmt::Formatter<'_>) -> ::core::fmt::Result {
            f.debug_tuple(::core::stringify!(ERC721URIStorage))
                .field(&self.address())
                .finish()
        }
    }
    ...
    impl ::ethers::contract::EthLogDecode for ERC721URIStorageEvents {
        fn decode_log(
            ...
            if let Ok(decoded) = BatchMetadataUpdateFilter::decode_log(log) {
                return Ok(ERC721URIStorageEvents::BatchMetadataUpdateFilter(decoded));
            }
            if let Ok(decoded) = MetadataUpdateFilter::decode_log(log) {
                return Ok(ERC721URIStorageEvents::MetadataUpdateFilter(decoded));
            }
    ...
    impl ::core::convert::From<BatchMetadataUpdateFilter> for ERC721URIStorageEvents {
        fn from(value: BatchMetadataUpdateFilter) -> Self {
            Self::BatchMetadataUpdateFilter(value)
        }
    }
    impl ::core::convert::From<MetadataUpdateFilter> for ERC721URIStorageEvents {
