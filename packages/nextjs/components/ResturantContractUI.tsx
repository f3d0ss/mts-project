...
import { ContractReadMethods } from "./scaffold-eth/Contract/ContractReadMethods";
import { ContractVariables } from "./scaffold-eth/Contract/ContractVariables";
import { ContractWriteMethods } from "./scaffold-eth/Contract/ContractWriteMethods";
 ...
 **/
export const ResturantContractUI = ({ index, selectedContract = "" }: ResturantContractUIProps) => {
  ...
  const deployedContractData = { address: resturantAddress, abi: contracts.ResturantToken.abi };
  ...
  if (!deployedContractData.address) {
    return (
      <p className="text-3xl mt-14">
        {`No contract found with the index "${index}" on chain "${configuredNetwork.name}"!`}
      </p>
    );
  }
          ...
          <div className="bg-base-100 border-base-300 border shadow-md shadow-secondary rounded-3xl px-6 lg:px-8 mb-6 space-y-1 py-4">
            <div className="flex">
              <div className="flex flex-col gap-1">
                <span className="font-bold">{resturantName as string}</span>
